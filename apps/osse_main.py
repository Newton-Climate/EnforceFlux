"""Legacy end-to-end OSSE driver.

Runs the pre-refactor single-JSON-config OSSE (source → instrument → transport →
inversion → metrics) in one shot. Kept for backward compatibility; new work
should use the per-stage drivers (``dispersion`` → ``instrument`` → ``flux`` →
``analysis``).
"""
